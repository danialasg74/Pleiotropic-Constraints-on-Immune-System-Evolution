############### Dmel Ids and seqs for immune genes ###############
library(Biostrings)

dmel_immun_Id = read.csv('Immunity_ncbi.csv')
dmel_immun_Id =  dmel_immun_Id$Protein.accession[dmel_immun_Id$Protein.accession != ""]

mel_cds = readDNAStringSet("dmel_immun.fasta")

###############  Ids 5 other species; immune genes ###############

setwd('../D_ananassae')
ana_immun_Id = read.csv('immun_match.csv')
ana_cds = readDNAStringSet("ana_immun.fasta")

setwd('../D_erecta')
ere_immun_Id = read.csv('immun_match.csv')
ere_cds = readDNAStringSet("ere_immun.fasta")

setwd('../D_sechellia')
sec_immun_Id = read.csv('immun_match.csv')
sec_cds = readDNAStringSet("sec_immun.fasta")

setwd('../D_simulans')
sim_immun_Id = read.csv('immun_match.csv')
sim_cds = readDNAStringSet("sim_immun.fasta")

setwd('../D_yakuba')
yak_immun_Id = read.csv('immun_match.csv')
yak_cds = readDNAStringSet("yak_immun.fasta")
...
for(i in 1:length(dmel_immun_Id)){
  ...
  IDs = data.frame(
    mel = key,
    ana = getV2(ana_immun_Id, key),
    ere = getV2(ere_immun_Id, key),
    sec = getV2(sec_immun_Id, key),
    sim = getV2(sim_immun_Id, key),
    yak = getV2(yak_immun_Id, key),
