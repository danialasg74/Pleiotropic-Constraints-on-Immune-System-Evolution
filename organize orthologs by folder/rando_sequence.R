############### Dmel Ids and seqs for random genes ###############
library(Biostrings)

dmel_rando_Id = read.delim('protid', header= FALSE)
dmel_rando_Id = dmel_rando_Id$V1

mel_cds = readDNAStringSet("dmel_rando.fasta")

###############  Ids 5 other species; random genes ###############

setwd('../D_ananassae')
ana_rando_Id = read.csv('rando_match.csv')
ana_cds = readDNAStringSet("ananas_rando.fasta")

setwd('../D_erecta')
ere_rando_Id = read.csv('rando_match.csv')
ere_cds = readDNAStringSet("ere_rando.fasta")

setwd('../D_sechellia')
sec_rando_Id = read.csv('rando_match.csv')
sec_cds = readDNAStringSet("sec_rando.fasta")

setwd('../D_simulans')
sim_rando_Id = read.csv('rando_match.csv')
sim_cds = readDNAStringSet("sim_rando.fasta")

setwd('../D_yakuba')
yak_rando_Id = read.csv('rando_match.csv')
yak_cds = readDNAStringSet("yak_rando.fasta")
...
for(i in 1:length(dmel_rando_Id)){
    ...
    IDs = data.frame(
      mel = key,
      ana = getV2(ana_rando_Id, key),
      ere = getV2(ere_rando_Id, key),
      sec = getV2(sec_rando_Id, key),
      sim = getV2(sim_rando_Id, key),
      yak = getV2(yak_rando_Id, key),
