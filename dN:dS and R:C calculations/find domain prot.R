...
get_subseq <- function(query_name, domain_tbl, protein_list) {
  ali_from <- domain_tbl$ali_from[domain_tbl$query == query_name]
  ali_to   <- domain_tbl$ali_to[domain_tbl$query == query_name]
  subseq(protein_list[[query_name]], start = ali_from, end = ali_to)
}
...
for (dir_indx in seq_along(dirs)) {
  ...
  if (file.exists('pro.fasta')) {
    protein <- readAAStringSet('pro.fasta')
  }
  ...
  if (!is.null(protein) && length(protein) >= 1 &&
    ...
    for (i in seq_along(pfams)) {
      ...
      for (j in seq_len(nrow(domain_tbl))) {
        ...
        if (!qname %in% names(protein)) next
        
        seq_j  <- get_subseq(qname, domain_tbl, protein)
        header <- paste0(">", qname)
      ...
      if (length(fasta_lines) > 0) {
        writeLines(fasta_lines, paste0(domain_name,'_pro',".fasta"))
      }
